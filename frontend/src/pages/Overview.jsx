...
export default function Overview(props) {
  ...
  const [notadoConfigured, setNotadoConfigured] = useState(false);
  ...
  useEffect(() => {
    GetSettings().then((settings) => {
      ...
      setNotadoConfigured(settings.notado_token !== "");
      setUploadStoreHighlights(settings.upload_store_highlights);
    });
  }, [settingsLoaded]);

  function promptNotado() {
    toast(
      "In order to upload to Notado, you need to configure your token on the Settings page!",
    );
  }

  function syncWithNotado() {
    const toastId = toast.loading("Preparing your highlights...");
    ForwardToNotado()
      .then((res) => {
        if (typeof res == "number") {
          toast.success(`Successfully forwarded ${res} highlights to Notado`, {
            id: toastId,
          });
        } else {
          toast.error(
      ...
      .catch((err) => {
        if (err.includes("401")) {
          toast.error(
            "Received 401 Unauthorised from Notado. Is your access token correct?",
            { id: toastId },
          );
        } else {
                  ...
                  <dl>
                    <div>
                      ...
                      <dd className="border-gray leading-6 font-medium text-black">
                        Sync your highlights with Notado
                      </dd>
                      <dt className="sr-only">Description</dt>
                      <dd className="text-xs text-gray-600">
                        Your Kobo is currently home to {highlightCounts.total}{" "}
                        highlights
                      </dd>
                      <dd className="text-xs text-gray-600">
                        {highlightCounts.sideloaded} sideloaded ·{" "}
                        {highlightCounts.official} official
                      </dd>
                    </div>
                  </dl>
                </button>
