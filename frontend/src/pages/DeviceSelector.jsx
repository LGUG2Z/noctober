  ...
  function detectDevices() {
    DetectKobos()
      .then((devices) => {
        ...
        toast.success(
          `${devices.length} kobo${devices.length > 1 ? "s" : ""} detected`,
          { id: "devices-found" },
        );
        setDevices(devices);
      })
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-800 flex flex-col">
      ...
      <div className="flex-grow items-center justify-center pb-24 px-24 grid grid-cols-2 gap-14">
        ...
        <div className="space-y-4 text-center">
          <h1 className="text-3xl font-bold dark:dark:text-gray-300">
            Select your Kobo
          </h1>
          <button onClick={detectDevices} className="dark:text-gray-400">
            Don't see your device? Click here to refresh device list.
          </button>
          <ul>
            {devices.map((device) => {
              ...
              if (!device.name) {
                description =
                  "Noctober did not recognise this Kobo but it's safe to continue";
              }
              return (
